...
# async def signup(dto: UserSignupDTO, session: AsyncSession):
#     # Google 사용자 정보 가져오기
...
async def update_me(user: User, dto: UserMeUpdateDTO, session: AsyncSession):
    for key, value in dto.dict(exclude_unset=True).items():
        setattr(user, key, value)
    #session.add(user) # 이미 세션에 있는 객체이므로 add는 필요 없음
    await session.commit()
    await session.refresh(user)
    return user

async def delete_me(user: User, session: AsyncSession):
